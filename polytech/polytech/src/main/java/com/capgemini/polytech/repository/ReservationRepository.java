package com.capgemini.polytech.repository;

import com.capgemini.polytech.entite.ReservationEntity;
import com.capgemini.polytech.entite.ReservationId;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

/**
 * Interface de gestion des opérations CRUD pour l'entité {@link ReservationEntity}.
 * Ce repository étend l'interface {@link JpaRepository}, qui fournit des méthodes
 * standard pour effectuer des opérations de persistance sur l'entité {@link ReservationEntity}.
 * Il est annoté avec {@link Repository} pour indiquer que c'est une couche de persistance.
 *
 * @see JpaRepository
 * @see ReservationEntity
 */
@Repository
public interface ReservationRepository extends JpaRepository<ReservationEntity, ReservationId> {

    // Aucune méthode spécifique n'est nécessaire ici, car JpaRepository fournit déjà les méthodes CRUD de base.
}
